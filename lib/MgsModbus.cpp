...
MgsModbus::MgsModbus(int port, int dataLen)
{
    this->port = port;
    this->dataLen = dataLen;
    //MbData = new word[dataLen];
}
...
void MgsModbus::Req(MB_FC FC, word Ref, word Count, word Pos)
{
  ...
  //****************** ?? ******************
  if (MbmClient.connect(ServerIp,this->port)) {
    #ifdef DEBUG
...
boolean MgsModbus::SetBit(word Number,boolean Data)
{
  int ArrayPos = Number / 16;
  int BitPos = Number - ArrayPos * 16;
  boolean Overrun = ArrayPos > this->dataLen * 16; // check for data overrun
  if (!Overrun){                 
